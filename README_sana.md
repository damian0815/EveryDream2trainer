...
Recommended for limited VRAM (24GB):
```bash
python /workspace/EveryDream2trainer-remote/train_sana.py \
    --model_id Efficient-Large-Model/SANA1.5_1.6B_1024px_diffusers \
    ...
    --resolution_mul 0.5 0.3 0.2 \
    --forward_slice_size 6 3 2 \
    --max_backward_slice_size 6 3 2 \
    --batch_size 6 \
    --optimizer_batch_size 24 \
    --lr 5e-4 \
    --max_epochs 10 \
    --save_every 1000000000 \
    --sample_prompts sample_prompts.json \
    --optimizer_config optimizer-adamw8bit.json \
    --unet_freeze_regex "freeze .*; unfreeze .*.attn1.*; unfreeze .*.attn2.*; unfreeze .*scale_shift_table.*; unfreeze caption.*"\
    --amp_without_grad_scaler \
    --plugins plugins.interruptible.InterruptiblePlugin \
    --seed -1 \
    --timesteps_multirank_stratified \
    --timesteps_multirank_stratified_distribution beta \
    --timesteps_multirank_stratified_alpha 3 \
