...
def load_sana_model(args: Namespace) -> SanaTrainingModel:
    ...
    #te_quantization = 'int8' # 'int8', 'int4', 'none
    te_quantization = 'none'
    pipe = _load_sana_pipeline(args.model_id, dtype=torch.bfloat16, te_quantization=te_quantization)
...
def _load_sana_pipeline(repo_id, dtype, te_quantization: Literal['none', 'int8', 'int4']= 'none') -> SanaPipeline:
    """
    Load a SanaPipeline with optional bitsandbytes quantization.
    ...
    if te_quantization in ("int8", "int4"):
        ...
        pipeline = SanaPipeline.from_pretrained(repo_id, **load_kwargs)
...
def save_sana_model(to_folder: str, model: SanaTrainingModel, global_step: int, num_samples: int) -> None:
    """
