...
def get_random_split(items: list, split_proportion: float, batch_size: int) \
        -> tuple[list, list]:
...
def _resolve_to_train_items(items: list, resolution: int) -> list[ImageTrainItem]:
    """
    Convert a list that may contain either ImageSourceItem or ImageTrainItem objects
    into a list of ImageTrainItem objects resolved at the given resolution.

    ImageTrainItem objects are passed through unchanged.
    ImageSourceItem objects are resolved via make_resolved_item().
    Items that are not feasible for the given resolution are skipped with a warning.
    """
    from data.image_train_item import ImageSourceItem
    import copy
    result = []
    for item in items:
        if isinstance(item, ImageSourceItem):
            opt = item.resolution_options.get(resolution)
            if opt is None or not opt.is_feasible:
                logging.warning(
                    f"Validation: {item.pathname} is not feasible at resolution {resolution}; "
                    f"skipping from validation split."
                )
                continue
            # deep-copy before mutating so the source pool is not corrupted
            result.append(copy.deepcopy(item).make_resolved_item(resolution))
        else:
            result.append(item)
    return result
...
class EveryDreamValidator:
    ...
    def _build_automatic_validation_dataset_if_required(self, image_train_items: list, model: TrainingModel) \
            -> tuple[Optional[ValidationDataset], list]:
        ...
        if val_split_mode is None or val_split_mode == 'none' or val_split_mode == 'manual':
            ...
            val_items_resolved = _resolve_to_train_items(val_items, self.resolution)
            val_items_resolved = list(disable_multiplier_and_flip(val_items_resolved))
            logging.info(f" * Removed {len(val_items)} images from the training set to use for validation")
            val_ed_batch = self._build_ed_batch(val_items_resolved, model=model, name='val')
    ...
    def _build_train_stabilizer_dataloader_if_required(self, image_train_items: list, model) \
        ...
        stabilize_items_resolved = _resolve_to_train_items(stabilize_items, self.resolution)
        stabilize_items_resolved = list(disable_multiplier_and_flip(stabilize_items_resolved))
        stabilize_ed_batch = self._build_ed_batch(stabilize_items_resolved, model=model, name='stabilize-train')
